...
type ScenarioQuestionCardProps = {
  question: ScenarioQuestionItem;
  index: number;
  onEdit: (question: ScenarioQuestionItem) => void;
  onDelete: (id: string) => void;
  isDeleting: boolean;
};

export function ScenarioQuestionCard({ question, index, onEdit, onDelete, isDeleting }: ScenarioQuestionCardProps) {
      ...
      <CardHeader className="relative pb-0 px-5 pt-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-1">
            <CardDescription>Scenario {index}</CardDescription>
            <CardTitle className="sm:text-lg text-sm leading-7 font-extrabold!">
              <span>Scenario : {question.question}</span>
            </CardTitle>
          </div>
