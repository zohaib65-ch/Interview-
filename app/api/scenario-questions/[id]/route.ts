...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    const updated = await ScenarioQuestion.findByIdAndUpdate(
      id,
    ...
    console.error("Failed to update scenario question", error);
    return NextResponse.json({ error: "Failed to update scenario question." }, { status: 500 });
  }
}
...
export async function DELETE(_request: Request, context: RouteContext) {
  try {
    ...
    await connectToDatabase();
    const deleted = await ScenarioQuestion.findByIdAndDelete(id);
    ...
    console.error("Failed to delete scenario question", error);
    return NextResponse.json({ error: "Failed to delete scenario question." }, { status: 500 });
  }
}
